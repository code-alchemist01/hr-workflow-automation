...
{
    public class AudioCreateDTOValidator : AbstractValidator<AudioCreateDTO>
    {
        private readonly string[] _allowedExtensions = { ".mp3", ".wav", ".ogg", ".m4a" };
        private const long MaxFileSize = 10 * 1024 * 1024; // 10MB

        public AudioCreateDTOValidator()
        {
            RuleFor(x => x.NoteId)
                .GreaterThan(0).WithMessage("Geçerli bir not ID'si gerekli");
